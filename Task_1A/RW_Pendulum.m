...
##*
##*  Team ID : DB_2395
##*  Team Leader Name: Nithi Santosh
##*  Team Member Name: Adithya Pradeep, Sona Philip, Mubeena KP
##*
##*  
...
function dy = RW_pendulum_dynamics(y, m1, m2, l1, wr, g, u)
  ...
  I2=(m2*wr^2)/2;
  ##dy(1,1) = y(2);
  ##dy(2,1) = (((m1*l1*0.5)+(m2*l1))*g*-1*sin(y(1)))/((m1*(l1*0.5)^2)+(m2*l1^2)+I1+I2);
  ##dy(3,1) = y(4);
  ##dy(4,1) = u/I2;
  ##
  d = [((m1*l1^2) + (m2*l1^2) + I1 + I2) I2; I2 I2];
  m_f = (m1*l1*0.5) + (m2*l1);
  dy(1,1) = y(2);
  dy(2,1) = -1*(d(2,2)*m_f*g*sin(y(1))/det(d)) - (d(1,2)*u/det(d));
  dy(3,1) = y(4);
  dy(4,1) = -1*(d(2,1)*m_f*g*sin(y(3))/det(d)) + (d(1,1)*u/det(d));
  ##
endfunction
...
function [A, B] = RW_pendulum_AB_matrix(m1 , m2, l1, wr, g)
  ...
  d = [((m1*l1^2) + (m2*l1^2) + I1 + I2) I2; I2 I2];
  m_f = (m1*l1*0.5) + (m2*l1);
  A = [0 1 0 0; (d(2,2)*m_f*g)/det(d) 0 0 0; 0 0 0 1; -1*(d(2,1)*m_f*g)/det(d) 0 0 0];
  B = [0; (-1*d(1,2))/det(d); 0; d(1,1)/det(d)];  
endfunction

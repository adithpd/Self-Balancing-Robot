...
function jacobian_matrices = find_jacobian_matrices(x_1, x_2, x1_dot, x2_dot)
  ...
  jacobian_matrices = {}; 
  ################## ADD YOUR CODE HERE ######################
  jacobian_intial = jacobian([x1_dot;x2_dot]);
  x_1 = transpose(x_1);
  x_2 = transpose(x_2);
  for i=[1:length(x_1)]
    jacobian_matrices{i} =  double(subs(jacobian_intial,{x1,x2},{x_1(i),x_2(i)}));
  endfor
  ############################################################  
endfunction
...
function [eigen_values stability] = check_eigen_values(x_1, x_2, jacobian_matrices)
  ...
  for k = 1:length(jacobian_matrices)
    ...
    for m = 1:length(eigen_values{k})
      if real(eigen_values{k}(m)) > 0
        flag = 0;
        break;
      endif
    endfor      
